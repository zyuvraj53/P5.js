...
var buildings = [];
const buildingLimit = 0;
var geo = new TexGeo();

let yellow, orange, salmon, offwhite, mint, cyan, electricPink, red, grayConcrete, brownConcrete, c1, c2, bgColor;
// Gradient variables 
let ox, oy, dx, dy;
let chaoticGrad = false;

function setup() {
    if (DEBUG_MODE) { setDeterministicSeed(); }
    ...
    if (decPairs[31] > 256) { chaoticGrad = true; }
...
function generateChildBuilding(buildingParent) {
    ...
    buildingSize = buildingSize.normalize().mult(random(buildingStartSizeMin, buildingStartSizeMax));
    ...
    buildingDrag = random(baseBuildingDrag * minDragFactor, baseBuildingDrag * maxDragFactor);
...
function setBuildingPos() {
    ...
    var buildingPos = createVector(random(buildingPosXMin, buildingPosXMax), random(buildingPosYMin, buildingPosYMax));
    return buildingPos;
}
...
function setBuildingSize() {
    ...
    var buildingSize = createVector(random(buildingSizeXMin, buildingSizeXMax), random(buildingSizeYMin, buildingSizeYMax));
    return buildingSize;
}
